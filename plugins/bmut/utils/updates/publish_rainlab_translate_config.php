...
class PublishRainlabTranslateConfig extends Migration
{
    ...
    public function up()
    {
        ...
        if (!File::isDirectory($fileDirectory)) {
            if (!File::makeDirectory($fileDirectory, 0777, true, true)) {
                throw new ApplicationException(Lang::get('bmut.utils::lang.errors.create_dir', [
                    'name' => $fileDirectory
                ]));
            }
        }

        if (@File::put($path, File::get(plugins_path('bmut/utils/classes/stubs/config/rainlab/translate/config.php'))) === false) {
            throw new ApplicationException(Lang::get('bmut.utils::lang.errors.create_file', [
                'file' => basename($path)
            ]));
        }
    ...
    public function down()
    {
        if (!Storage::createLocalDriver(['root' => config_path()])->deleteDirectory('rainlab')) {
            throw new ApplicationException(Lang::get('bmut.utils::lang.errors.delete_directory'));
        }
    }
}